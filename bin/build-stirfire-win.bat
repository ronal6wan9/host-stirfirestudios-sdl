...
cd %~dp0%..
set rootpath=%cd%
set defaultprefix=%rootpath%\lib\stirfire\%arg1%
set libprefix=%2
if "%libprefix%"=="" set libprefix=%defaultprefix%
set hostpath=%3
if "%hostpath%"=="" set hostpath=host-stirfire
...
-DCUSTOM_HOST=%hostpath% ^
%rootpath%\cmake\hosts\%hostpath% || exit /b 1
...
cmake -DCMAKE_INSTALL_PREFIX=%libprefix%\Debug -DCUSTOM_HOST=%hostpath% %rootpath%\cmake\hosts\%hostpath% || exit /b 1
...
copy /Y %libprefix%\Release\bin\sledge.exe %libprefix%\Distribute\bin\sledge.exe


if NOT EXIST %rootpath%\util\sledge.exe copy /Y %libprefix%\Release\bin\sledge.exe %rootpath%\util\sledge.exe
